...
import { Board, GradientBackground, MyButton, MyText } from "../../components";
...
export default function MultiplayerGame({ navigation, route }: MultiPlayerGameProps): ReactElement {
    ...
    const initGame = async () => {
        ...
        try {
            ...
            Alert.alert("Error!", getErrorMessage(typedError));
        }
        setLoading(false);
    };
            ...
            const subscription = gameUpdates.subscribe({
                next: ({ value }) => {
                    ...
                    if (newGame && game) {
                        ...
                        setGame({ ...game, status, state, winner, turn });
                        if (user) {
                            user.username === turn ? playSound("player1") : playSound("player2");
                        }
                    }
                }
            });
            ...
            <SafeAreaView style={styles.container}>
                {loading && (
                    <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
                        <ActivityIndicator color={"#e6e6ff"} />
                    </View>
                )}
                {game && user && (
                    <>
                        <View style={{ width: SCREEN_WIDTH - 60 }}>
                            <MyText style={styles.turn} numberOfLines={1}>
